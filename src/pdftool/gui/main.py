...
class ModernPDFTool:
    ...
    def create_header(self, parent):
        ...
        title_label = ttk.Label(header_frame, text=settings.app_name, style="Title.TLabel")
        title_label.grid(row=0, column=0)

        subtitle_label = ttk.Label(
            header_frame, text="专业的PDF操作工具 - 合并、拆分、信息提取", style="Subtitle.TLabel"
        )
    ...
    def create_merge_tab(self):
        ...
        ttk.Button(merge_buttons, text="添加PDF文件", command=self.add_merge_files).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(merge_buttons, text="移除选中", command=self.remove_merge_files).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(merge_buttons, text="清空列表", command=self.clear_merge_files).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            merge_buttons, text="合并PDF", command=self.merge_pdfs, style="Accent.TButton"
    ...
    def create_split_tab(self):
        ...
        self.split_file_entry = ttk.Entry(
            file_frame, textvariable=self.split_file_var, state="readonly"
        )
        ...
        ttk.Radiobutton(
            options_frame, text="每页单独拆分", variable=self.split_mode, value="all"
        ).grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Radiobutton(
            options_frame, text="指定页面范围", variable=self.split_mode, value="range"
        ...
        ttk.Entry(range_frame, textvariable=self.start_page_var, width=8).grid(
            row=0, column=1, padx=(5, 15)
        )

        ttk.Label(range_frame, text="结束页:").grid(row=0, column=2)
        self.end_page_var = tk.StringVar()
        ttk.Entry(range_frame, textvariable=self.end_page_var, width=8).grid(
            row=0, column=3, padx=5
        )

        # Split button
        ttk.Button(
            split_frame, text="拆分PDF", command=self.split_pdf, style="Accent.TButton"
    ...
    def create_info_tab(self):
        ...
        self.info_file_entry = ttk.Entry(
            file_frame, textvariable=self.info_file_var, state="readonly"
        )
        ...
        ttk.Button(
            file_frame, text="获取信息", command=self.get_pdf_info, style="Accent.TButton"
        ...
        self.info_text = tk.Text(
            info_display_frame, wrap=tk.WORD, state=tk.DISABLED, font=("Consolas", 10)
        )
    ...
    def add_merge_files(self):
        ...
        files = filedialog.askopenfilenames(
            title="选择PDF文件", filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")]
        )
    ...
    def merge_pdfs(self):
        ...
        output_file = filedialog.asksaveasfilename(
            title="保存合并后的PDF", defaultextension=".pdf", filetypes=[("PDF files", "*.pdf")]
        )
    ...
    def select_split_file(self):
        ...
        file_path = filedialog.askopenfilename(
            title="选择要拆分的PDF文件", filetypes=[("PDF files", "*.pdf"), ("All files", "*.*")]
        )
        if file_path:
