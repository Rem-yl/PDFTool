...
from .models import PDFInfo, SplitOptions, MergeOptions, ExtractOptions, PageSelectionOptions, OperationResult, SplitMode, PageSelectionMode
    ...
    def select_pages(self, input_file: Path, options: PageSelectionOptions) -> OperationResult:
        ...
        try:
            with open(input_file, 'rb') as f:
                ...
                elif options.mode in [PageSelectionMode.SPECIFIC_PAGES, PageSelectionMode.SINGLE_FILE]:
                    if not options.pages:
                ...
                pages_desc = f"{min(target_pages)}-{max(target_pages)}" if len(target_pages) > 1 else str(target_pages[0])
